...
export const updateDirectory = async ({
  ...
  name?: string | null;
  parentId?: string | null;
}): Promise<{ result: DirectoryRecord }> => {
  ...
  if (nextName) {
    const parentForCheck =
      parentId === undefined ? (current.parent_id ?? null) : nextParent;
    const existing = await get("file_directories")
      .select(["id"])
      .where("name", "=", nextName)
  ...
  const updatePayload: Partial<DirectoryRecord> = {};
  if (nextName !== null) {
...
const collectDescendants = (rootId: string, directories: DirectoryRecord[]) => {
  const byParent = new Map<string | null, DirectoryRecord[]>();

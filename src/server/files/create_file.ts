...
export const createFile = async ({ fileData }: { fileData: CreateFileInput }): Promise<CreateFileResult> => {
  ...
  const contentType = getContentType(parsedFileData.file);

  return {
    fileUrl,
    objectKey,
    originalName: parsedFileData.originalName,
    contentType,
    size: parsedFileData.file.size,
    directoryId: parsedFileData.directoryId,
    storageFolder: parsedFileData.storageFolder,
    uploaderId: parsedFileData.uploaderId,
  };
};
...
const isFileLike = (value: unknown): value is FileLike => typeof Blob !== "undefined" && value instanceof Blob;
...
const normalizePayload = (fileData: CreateFileInput): NormalizedFilePayload => {
  ...
  if (!isFileLike(maybeFile) || maybeFile.size === 0) {
    throw new Error("Yuklenecek gecerli bir dosya bulunamadi");
  }

  const directoryIdValue = typeof fileData.directoryId === "string" ? fileData.directoryId.trim() : "";
  const storageFolderValue = typeof fileData.storageFolder === "string" ? fileData.storageFolder : undefined;
...
const uploadToObjectStorage = async (payload: NormalizedFilePayload): Promise<string> => {
...
const getContentType = (file: FileLike) => (file.type && file.type.trim().length > 0 ? file.type : "application/octet-stream");

...
export async function up(db: Kysely<DatabaseConnection>): Promise<void> {
  await withUuid(
    db.schema
      ...
      .addColumn("status", "text", (col) =>
        col.notNull().defaultTo("unverified"),
      )
  ...
  await withUuid(
    db.schema
      ...
      .addColumn("parent_id", "uuid", (col) =>
        col.references("file_directories.id").onDelete("cascade"),
      )
      .addColumn("created_at", "timestamp", (col) =>
        col.defaultTo(sql`CURRENT_TIMESTAMP`).notNull(),
      )
  ...
  await withUuid(
    db.schema
      ...
      .addColumn("directory_id", "uuid", (col) =>
        col.references("file_directories.id").onDelete("set null"),
      ),
  ).execute();
  ...
  await withUuid(
    db.schema
      ...
      .addColumn("site_map_id", "uuid", (col) =>
        col.references("site_map.id").onDelete("set null"),
      )
      .addColumn("created_at", "timestamp", (col) =>
        col.defaultTo(sql`CURRENT_TIMESTAMP`).notNull(),
      )
  ...
  await db.schema
    ...
    .addForeignKeyConstraint(
      "site_map_active_page_id_pages_id_fk",
      ["active_page_id"],
      "pages",
      ["id"],

...
export const mapDbFileToSerializable = (row: any): SerializableFileRecord => ({
  id: row.id,
  url: row.url,
...
export const mapDbDirToSerializable = (row: any): SerializableDirectoryRecord => ({
  id: row.id,
  name: row.name,
...
export async function fetchFiles(includeDeleted = true) {
  ...
  return (data.result ?? []).map(mapDbFileToSerializable) as SerializableFileRecord[];
}

export async function fetchDirectories() {
  const res = await fetch(`/api/files/directories`);
  const data = await res.json();
  return (data.result ?? []).map(mapDbDirToSerializable) as SerializableDirectoryRecord[];
}

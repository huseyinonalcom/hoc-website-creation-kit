...
export function FilesBrowserClient({
  ...
  const visibleDirectories = useMemo(() => {
    ...
    return directories.filter((dir) => (dir.parentId ?? null) === activeDirectoryId).sort((a, b) => a.name.localeCompare(b.name, "tr"));
  ...
  const parentDirectoryId = activeDirectoryId && directoryMap.get(activeDirectoryId) ? (directoryMap.get(activeDirectoryId)!.parentId ?? null) : null;

  const handleSelect = (file: SerializableFileRecord, index: number, event: MouseEvent) => {
    if (!multiSelect) {
  ...
  const handleDragOverTarget = (targetId: string | null) => (event: DragEvent) => {
    if (!enableDragDrop) return;
  ...
  const handleDropTarget = (directoryId: string | null) => (event: DragEvent) => {
    ...
    <div className={cn("space-y-6", className)}>
      <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900">
        <header className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-gray-900 dark:text-white">{showFiles ? "Dosyalar" : "Klasorler"}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {canCreateDirectory ? (
              <Button type="button" variant="outline" onClick={() => setIsCreateModalOpen(true)}>
                Klasor Olustur
              </Button>
            ) : null}
            {canUpload ? (
            ...
            {activeDirectoryId ? (
              <button
                className={cn(
                  ...
                  dragOverTarget === "__up__" ? "border-indigo-500 bg-indigo-50 dark:border-indigo-400 dark:bg-indigo-500/10" : null,
                )}
                type="button"
              ...
              ? visibleDirectories.map((directory) => (
                  <button
                    ...
                    className={cn(
                      ...
                      dragOverTarget === directory.id ? "border-indigo-500 bg-indigo-50 dark:border-indigo-400 dark:bg-indigo-500/10" : null,
                    )}
                    type="button"
      ...
      <Dialog className="relative z-50" open={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-gray-900">
              <div className="mb-4 flex items-center justify-between">
                <DialogTitle className="text-lg font-semibold text-gray-900 dark:text-white">Klasor Olustur</DialogTitle>
              ...
              <div className="space-y-3">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-200" htmlFor="directory-name">
                  Klasor Adi
                </label>
                <input
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  <Button disabled={isCreatingDirectory} type="button" onClick={handleCreateDirectory}>
      ...
      <Dialog className="relative z-50" open={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)}>
        <div aria-hidden="true" className="fixed inset-0 bg-black/40" />
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-gray-900">
              <div className="mb-4 flex items-center justify-between">
                <DialogTitle className="text-lg font-semibold text-gray-900 dark:text-white">Dosya Yukle</DialogTitle>
              ...
              <div className="space-y-3">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-200" htmlFor="file">
                  Dosya
                </label>
                <input
                  ref={fileInputRef}
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {uploadState.result === "error" && uploadState.error && <p className="text-red-600">{uploadState.error}</p>}
                  {uploadState.result === "success" && uploadState.uploadedFile ? (
                    <p className="text-green-600">{getDisplayName(uploadState.uploadedFile)} basariyla eklendi.</p>
                  ) : null}
                </div>
                <div className="flex items-center justify-end gap-2">

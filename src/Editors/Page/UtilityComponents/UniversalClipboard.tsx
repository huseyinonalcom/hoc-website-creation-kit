  ...
  const handlePaste = async () => {
    ...
    try {
      ...
      const sanitized = isPlainObject(parsed?.value)
        ? stripId(parsed.value as Record<string, unknown>)
        : null;
      if (!sanitized) {
        scheduleStatusReset("error");
        return;
      ...
      dispatch({
        ...
        data: (previous) => {
          const updateItemProps = (
            item: ComponentData<Record<string, unknown>>,
          ) => ({
            ...item,
            props: {
          ...
          if (selector.zone && previous.zones?.[selector.zone]) {
            ...
            const updatedZone = zoneItems.map((item, index) =>
              index === selector.index
                ? updateItemProps(
                    item as ComponentData<Record<string, unknown>>,
                  )
                : item,
            );
          ...
          const updatedContent = previous.content.map((item, index) =>
            index === selector.index
              ? updateItemProps(item as ComponentData<Record<string, unknown>>)
              : item,
          );

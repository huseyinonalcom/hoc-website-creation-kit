...
import type { GalleryBlockItem, GalleryBlockItemSize, GalleryGridSize, GalleryImageMode } from "./type";
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-3">
          {(["cover", "contain"] as const).map((mode) => (
            <button key={mode} className={buttonClasses(currentValue === mode)} type="button" onClick={() => onChange(mode)}>
    ...
    items: {
      ...
      arrayFields: {
        ...
        imageUrl: {
          label: "Görsel",
          type: "custom",
          render: ({ value, onChange }) => <EditorImage value={typeof value === "string" ? value : ""} onChange={(next) => onChange(next)} />,
        },
        href: {

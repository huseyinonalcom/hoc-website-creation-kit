...
export const imageWithSlotConfig: Config<BaseEditorProps>["components"]["ImageWithSlot"] =
  {
    ...
    render: ({
      ...
      const mobileOrderClass =
        stackOrder === "content-first" ? "flex-col-reverse" : "flex-col";
      const resolvedHeight =
        typeof height === "number" && height > 0 ? height : 360;
      ...
      return (
        <div
          className={`flex ${mobileOrderClass} gap-6 lg:items-center ${layoutClass}`}
        >
          {imageNode ? (
            hasHref ? (

...
export const getUsers = authCheckMiddleware({
    allowedRoles: ["admin"],
    fn: async ({ nameFilter, count = 30, page = 1, }) => {
        ...
        const safePage = Number.isFinite(page) && page && page > 0
            ? Math.floor(page)
            : 1;
        ...
        const total = await buildFilteredQuery()
            .select(dbRo.fn.count("id").as("totalCount"))
            .executeTakeFirstOrThrow()
            .then((res) => res.totalCount);
        const resultWithoutPasswords = result.map(({ password_hash: _password_hash, ...user }) => user);
        return {

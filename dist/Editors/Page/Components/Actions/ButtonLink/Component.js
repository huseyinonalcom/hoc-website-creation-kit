...
export const ButtonLink = ({ text, url, icon, openInNewTab, color, borderRadius, }) => {
    const normalizedHtml = (text ?? "").trim();
    const IconComponent = buttonIconOptions.find((option) => option.value === icon)?.Icon ??
    ...
    return (_jsxs(Link, { className: "inline-flex items-center gap-3 rounded-full px-6 py-3 text-sm font-semibold text-white", href: resolvedHref, rel: rel, style: {
            backgroundColor,

...
export function LinkBar({ links = [] }) {
    ...
    return (_jsx("nav", { className: "hidden w-full flex-row items-center justify-center gap-6 bg-white pt-4 md:flex dark:bg-gray-800", children: links
            ...
            return (_jsx(Link, { className: cn("border-b-3 pb-4 text-lg font-semibold transition-colors", isActive
                    ? "border-blue-600 text-blue-600"
                    : "border-transparent hover:border-blue-600 hover:text-blue-600"), href: link.path, children: link.label }, `${link.path}-${idx}`));
        }) }));
}

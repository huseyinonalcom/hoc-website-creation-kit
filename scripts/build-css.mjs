...
const puckDarkThemePath = path.resolve(__dirname, "..", "src", "puck-dark-theme.css");
...
const buildCombinedCss = ({ puckCss, quillCss, tableCss, puckDarkCss }) => {
...
const sync = async () => {
  ...
  const puckDarkCss = await readFile(puckDarkThemePath, "utf8");

  await writeFile(combinedTargetPath, buildCombinedCss({ puckCss, quillCss, tableCss, puckDarkCss }), "utf8");

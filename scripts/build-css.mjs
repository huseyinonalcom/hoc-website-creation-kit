...
const tailwindGeneratedPath = path.resolve(__dirname, "..", "src", "tailwind.generated.css");
...
const buildCombinedCss = ({ tailwindCss, puckCss, quillCss, tableCss }) => {
  const header = "/* Generated by scripts/build-css.mjs. Do not edit manually. */\n";
  return [header, tailwindCss.trimEnd(), "", puckCss.trimEnd(), "", quillCss.trimEnd(), "", tableCss.trimEnd(), ""].join("\n");
};

const sync = async () => {
  const tailwindCss = await readFile(tailwindGeneratedPath, "utf8");
  ...
  await writeFile(combinedTargetPath, buildCombinedCss({ tailwindCss, puckCss, quillCss, tableCss }), "utf8");
  if (existsSync(tailwindGeneratedPath)) {
    await unlink(tailwindGeneratedPath);
  }
